...
display.checkIfSpriteIsBlank = function(spriteName) {
  // ACK this is SO SLOW, gosh how valuable is this actually tho
...
display.calculateSpritesheet = (imageName, spriteWidth, spriteHeight) => {
  ...
  for (let i = 0; i < nY; i++) {
    for (let j = 0; j < nX; j++) {
      ...
      display.checkIfSpriteIsBlank(spriteName);
    }
  }
  return nX * nY;
};
...
display.updatePictureSpriteSize = function(name, spriteWidth, spriteHeight) {
  ...
  if (Array.isArray(picture)) {
    ...
    picture.spriteWidth = spriteWidth || 64;
    picture.spriteHeight = spriteHeight || 64;
    display.filterPictureSprites(name, spriteWidth, spriteHeight);
  }
};

display.filterPictureSprites = function(name, spriteWidth, spriteHeight) {
  const picture = display.pictures[name];
  if (picture) {
...
display.setLoop = function(cb) {
  ...
  const _loop = () => {
    if (display.stopLoop) {
      display.stopLoop = false;
      display.looping = false;
      if (display.onStopLoop) {
        display.onStopLoop();
      }
      return;
    }
