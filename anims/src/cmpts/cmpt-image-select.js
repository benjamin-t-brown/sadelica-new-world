...
import { colors, getImageFilter, setImageFilter } from '../utils';
...
const ImageSelect = props => {
  const [filter, setFilter] = React.useState(getImageFilter());
  ...
  const filteredImages = Object.keys(display.pictures).filter(imageName => {
    if (filter) {
      return (
        imageName !== 'invisible' &&
    ...
    >
      ...
      >
        <input
          ...
          value={filter}
          onChange={ev => handleSetFilter(ev.target.value)}
        ></input>
        <Button
          type="cancel"
      ...
      >
        ...
        {filteredImages.length === 0 ? (
          <div style={{ margin: '16px' }}>No images found</div>
        ) : null}
      </div>
    </div>
  );
