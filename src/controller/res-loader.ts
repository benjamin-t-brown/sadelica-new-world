...
import { addCadence, loadImagesAndSprites } from 'model/canvas';
...
class AssetLoader {
  async processAssetFile(text: string): Promise<void> {
    ...
    const _Picture = async function (line: any) {
      ...
      const { imageMap } = await loadImagesAndSprites([
        [
          pictureName,
          url,
          Number(spriteWidth),
    ...
    const _Cadence = function (line) {
      const [, cadenceName, spr1, spr2, spr3] = line;
      addCadence(cadenceName, spr1, spr2, spr3);
    };
...
export const loadRes = async (loadingTick?: () => void) => {
  ...
  // const songLoop = await (await fetch(PATH_PREFIX + 'snd/songLoop.txt')).json();

  // setSoundLoop(songLoop);
